...
void IChatbox::HookCommand()
{
    ...
    Tools->Intercept(ITools::_I_CALL, (void*)0x007089E5, Hooks::KCommand::ProcessCommand, 5); // 100%
    Tools->Intercept(ITools::_I_CALL, (void*)0x007BC533, Hooks::KCommand::ProcessCommand, 5); // 100%
}
...
void IChatbox::AddEditorText(std::string Text)
{
	...
	if (*Engine::KGameSys::m_pChattingControl)
	{
		...
		if (m_Edit)
		{
			void (__thiscall *OnKeyDown)(void* _this, unsigned int) = (void (__thiscall*)(void*, unsigned int)) ((*(unsigned long*)((*(unsigned long*)m_Edit)+0x6c)));
