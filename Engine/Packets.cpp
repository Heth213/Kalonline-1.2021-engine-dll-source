...
std::map<int,ConfigMonster> ShowMonsterAmount; int QuestIDCheck = 0, AutoPick = 0;
bool StopClient = false, ClientProtection = false, AsadalBuffCheck = false; int AsadalValue = 1; std::string ID = "empty", PWD = "empty";
static int (__cdecl *QuestIDGet)(int QuestID) = (int (__cdecl*)(int))0x006D1E00; // 2021
...
{
	if (QuestIDCheck && ShowMonsterAmount.find(*(DWORD*)0x00919670)->second.Quest == QuestIDCheck)//2021
		sprintf(Msg, Text, Text2, ShowMonsterAmount.find(*(DWORD*)0x00919670)->second.Amount, MaxAmount);//2021
	else
...
void IPackets::Hook()
{
	...
	OnSend->Hook();
	int Time = 3600;
    Tools->MemcpyEx((void*)0x0057697A, &Time, 4); // 2021 100% 
    Tools->Intercept(ITools::_I_CALL, (void*)0x00694995, Hooks::KSocket::OnRecv, 5);// 2021 100%
    Tools->Intercept(ITools::_I_CALL, (void*)0x004CE680, (void*)AsadalCheck, 5);// 2021 100%
    Tools->Intercept(ITools::_I_CALL, (void*)0x0077F6AB, (void*)AsadalCheck, 5);// 2021 100%
    Tools->Intercept(ITools::_I_CALL, (void*)0x00624351, (void*)GetQuestID, 5);// 2021 100%
    Tools->Intercept(ITools::_I_CALL, (void*)0x00624A55, (void*)GetMsg, 5);// 2021 100%
}
...
int IPackets::Recv(Engine::Packet *Data)
{
	...
	if ((int)Data->Type == 144)
	{
		...
		if (!Damage && CheckDefender && Defender == *(int*)((*Engine::CGame_Character::m_Master) + 17460))
			Engine::KBattle_UI::DrawOnCharacter((void*)0x00A10A60, 7, 0, Defender);//2021

		if (!Damage && CheckAttacker && Attacker == *(int*)((*Engine::CGame_Character::m_Master) + 17460))
			Engine::KBattle_UI::DrawOnCharacter((void*)0x00A10A60, 5, 0, Defender);//2021
	}
	...
	if (Data->Type == 0xFF)
	{
		...
		if (Type == 255)
		{
			...
			if (strlen(xClient) > 0 && strlen(xEngine) > 0 && strlen(xConfig) > 0 && strlen(xMConfig) > 0 && strlen(xE) > 0)
			{
				if ((std::string)xClient != (std::string)Client.digestFile("Engine.dll") && StopClient != true)
				{
					StopClient = true;
					Engine::KGameSys::MBox((int)"Out dated files detected. Please update your client.",0,0,0,0);
					return 1;
				}

				if ((std::string)xEngine != (std::string)Engine.digestFile("engine.exe") && StopClient != true)
				{
					StopClient = true;
					Engine::KGameSys::MBox((int)"Out dated files detected. Please update your client.",0,0,0,0);
					return 1;
				...
				if ((std::string)xConfig != (std::string)Config.digestFile("data/Config/config.pk") && StopClient != true)
				{
					StopClient = true;
					Engine::KGameSys::MBox((int)"Out dated files detected. Please update your client.",0,0,0,0);
					return 1;
				...
				if ((std::string)xMConfig != (std::string)MConfig.digestFile("data/Config/m_config.pk") && StopClient != true)
				{
					StopClient = true;
					Engine::KGameSys::MBox((int)"Out dated files detected. Please update your client.",0,0,0,0);		
					return 1;
				...
				if ((std::string)xE != (std::string)E.digestFile("data/HyperText/nui/script.pk") && StopClient != true)
				{
					StopClient = true;
					Engine::KGameSys::MBox((int)"Out dated files detected. Please update your client.",0,0,0,0);	
					return 1;
		...
		if (Type == 242)
		{
			...
			Engine::KGameSys::ScreenOverlay((int)0x00927620, Overlay, 1077936128, RGBA2DWORD(Colorx,Colory,Colorz,255), 0);
			return 1;
		}
		...
		//{
		//	StopClient = true;
		//	Engine::KGameSys::MBox((int)"Packet hack detected. Client connection closed.",0,0,0,0);	
		//	return 1;
		//}
